...
int newSockListen(unsigned int port, char *listen_ip)
{
	...
	{
		wlog_err("ERREUR: socket().. (%s line: %i)\n",__FILE__, __LINE__);
		return -1;
	}
	...
	{
		wlog_err("ERREUR: bind(%i) (non-root ?).. (%s line: %i)\n", port, __FILE__, __LINE__);
		return -3;
	}

	if (listen(sock, 2048) == -1)
	{
		wlog_err("ERREUR: listen().. (%s line: %i)\n",__FILE__, __LINE__);
		return -4;
	}
...
static void check_idle(struct _socks_list *sl)
{
	...
	for (i = 0; x < *sl->tfd; i++) {
		//wlog_dbg("check %d of %d connect", i, *sl->tfd);
...
unsigned int sockroutine(int s_listen, acetables *g_ape)
{
	...
	if (epoll_fd < 0) {
		wlog_err("[ERR] Not enougth memory\n");
		exit(0);		
	}
