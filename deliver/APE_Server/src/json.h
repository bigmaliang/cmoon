...
#define JNEED_STR_CPY(item, key, val)                               \
    do {                                                            \
        if (item == NULL || json_lookup(item, key) == NULL)         \
            return (RETURN_BAD_PARAMS);                             \
        val = strdup(json_lookup(item, key)->jval.vu.str.value);    \
    } while(0)
#define JNEED_STR_CPY_VOID(item, key, val)                          \
    do {                                                            \
        if (item == NULL || json_lookup(item, key) == NULL)         \
            return;                                                 \
        val = strdup(json_lookup(item, key)->jval.vu.str.value);    \
...
#define JNEED_INT(item, key, val)                               \
    do {                                                        \
        if (item == NULL || json_lookup(item, key) == NULL)     \
            return (RETURN_BAD_PARAMS);                         \
        val = json_lookup(item, key)->jval.vu.integer_value;    \
    } while(0)
#define JNEED_INT_VOID(item, key, val)                          \
    do {                                                        \
        if (item == NULL || json_lookup(item, key) == NULL)     \
            return;                                             \
        val = json_lookup(item, key)->jval.vu.integer_value;    \
...
#define JNEED_FLOAT(item, key, val)                         \
    do {                                                    \
        if (item == NULL || json_lookup(item, key) == NULL) \
            return (RETURN_BAD_PARAMS);                     \
        val = json_lookup(item, key)->jval.vu.float_value;  \
    } while(0)
#define JNEED_FLOAT_VOID(item, key, val)                    \
    do {                                                    \
        if (item == NULL || json_lookup(item, key) == NULL) \
            return;                                         \
        val = json_lookup(item, key)->jval.vu.float_value;  \
...
#define JNEED_OBJ(item, key, val)                           \
    do {                                                    \
        if (item == NULL || json_lookup(item, key) == NULL) \
            return (RETURN_BAD_PARAMS);                     \
        val = json_lookup(item, key);                       \
    } while(0)
#define JNEED_OBJ_VOID(item, key, val)                      \
    do {                                                    \
        if (item == NULL || json_lookup(item, key) == NULL) \
            return;                                         \
        val = json_lookup(item, key);                       \
