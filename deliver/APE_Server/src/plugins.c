...
ace_plugins *loadplugin(char *file)
{
	...
	if (!hwnd) {
		wlog_err("[Module] Failed to load %s [Invalid library] (%s)\n", file, dlerror());
		return NULL;
	}
...
void findandloadplugin(acetables *g_ape)
{
	...
	for (i = 0; i < globbuf.gl_pathc; i++) {
		...
		if (pcurrent != NULL) {
			...
			if (load == NULL) {
				wlog_err("[Module] Failed to load %s [No load entry point]\n", globbuf.gl_pathv[i]);
				free(pcurrent);
				continue;
			}
...
void plugin_read_config(ace_plugins *plug)
{
	if (plug->infos->conf_file != NULL) {
		if ((plug->infos->conf = plugin_parse_conf(plug->infos->conf_file)) == NULL) {
			wlog_err("[Module] [%s] [WARN] Cannot open configuration (%s)\n", plug->modulename, plug->infos->conf_file);
			return;			
		}
	}
}
