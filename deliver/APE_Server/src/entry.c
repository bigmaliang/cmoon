...
int main(int argc, char **argv) 
{
	...
	if (CONFIG_VAL(Server, log_file, srv) != NULL) {
		if (log_init(CONFIG_VAL(Server, log_file, srv)) != 1) {
			printf("\nOpen log file %s failure\n", CONFIG_VAL(Server, log_file, srv));
			exit(1);
		}
	} else {
		log_init("-");
	}
	if (CONFIG_VAL(Server, log_level, srv) != NULL) {
		log_setlv(atoi(CONFIG_VAL(Server, log_level, srv)));
	}
