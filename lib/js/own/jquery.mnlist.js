;(function($) {
	$.fn.mnlist = function(rows, opts) {
		opts = $.extend(
		{
			bindTo: document,
			title: "listtitle",
			time: "1970-01-01",
			titleClickName: "",
			titleClickHref: "#click",
			titleClickCbk: function() {return true;},
			toggleAble: true,
			content: "listcontent"
		}, opts||{});

		if (type(rows) != "Array") {
			return null;
		}

		var retobj = $("<ul id='mnlist'></ul>").appendTo($(opts.bindTo));
		retobj.num = 0;

		function makeList(rows)
		{
			$.each(rows, function(i, row)
			{
				var e_item = $("<li></li>").appendTo(retobj);
				var e_title = $("<div class='title'>"+row[opts.title]+"</div>").
					appendTo(e_item);
				$("<span class='lstTime'> "+row[opts.time]+"</span>").
					appendTo(e_title);
				if (opts.titleClickName) {
					$("<a href='"+opts.titleClickHref+"'>"+
					  opts.titleClickName+"</a>").click(opts.titleClickCbk(row)).
					  appendTo(e_title);
				}
				if (opts.toggleAble) {
					var clc = $("<a href='javascript:void(0);' class='lstToggle'>&lt;&lt;</a>").
						click(function()
						{
							e_item.toggle("slow");
						}).appendTo(e_title);
					clc.facebox();
				}
				if (type(opts.content) == "String") {
					$("<li class='content'>"+row[opts.content]+"</li>").
						appendTo(e_item);
				} else if (type (opts.content) == "Function"){
					opts.content(row, e_item);
				}
				retobj.num++;
			});
		}

		retobj.makeList = makeList;
		makeList(rows);
		return retobj;
	 };
})(jQuery);
